...
class MainWindow(QtWidgets.QWidget):
    ...
    def page1(self):
        page = uic.loadUi("tvshow_page1.ui")

        data = parseData.tvshow_stats()
    ...
    def page2(self):
        graph1, graph2, graph3 = parseData.tvshow_graphs()

        page = uic.loadUi("tvshow_page2.ui")
    ...
    def page3(self):
        top_show, name, most_played_shows = parseData.top_tv_shows()

        page = uic.loadUi("tvshow_page3.ui")
        ...
        posters = [top_show[show]['poster'] for show in top_show]
        playtimes = [top_show[show]['playtime'] for show in top_show]

        n = 1
        for id, poster, playtime in zip(top_show.keys(), posters, playtimes):
        ...
        for n, id in enumerate(most_played_shows.keys(), 1):

            show_title_widget = page.findChild(QtWidgets.QLabel, f"showname{n}")
            plays_widget = page.findChild(QtWidgets.QLabel, f"plays{n}")

            show_title = most_played_shows[id]['show']['title']

            plays = most_played_shows[id]['play_count']
            ...
            show_title_widget.setText(show_title)
            plays_widget.setText(plays_label)

        return page

    def page4(self):
        genres, graph = parseData.tv_shows_stats2()

        page = uic.loadUi("tvshow_page4.ui")
        ...
        for genre, value in genres.items():
            ...
            label.setText(richtext("HOME",f"<html><head/><body><p style='margin-bottom:0; font-weight:600;'>{genre}</p><p style='margin : 0; padding-top:0; font-size:7pt;'>{value} SHOWS</p></body></html>"))
    ...
    def page5(self):
        page = uic.loadUi("tvshow_page5.ui")

        data, countries = parseData.tv_show_countries()
    ...
    def page6(self):
        ...
        page = uic.loadUi('tvshow_page6.ui')
        networks = parseData.tv_show_networks()
    ...
    def page7(self):
        ...
        page = uic.loadUi("tvshow_page7.ui")
        graph = parseData.tv_list_progress()
        page.graph.setupUi(graph)

        return page

    def page8(self):
        highest_rated, all_ratings = parseData.highest_rated_tv()

        ratings_quote = {10: 'TOTALLY NINJA!', 9: 'SUPERB', 8: 'GREAT', 7: 'GOOD', 6: 'FAIR', 5: 'MEH', 4: 'POOR', 3: 'BAD', 2: 'TERRIBLE', 1: 'WEAK SAUCE :('}

        page = uic.loadUi("tvshow_page8.ui")
        ...
        for rating, value in all_ratings.items():
            ...
            label = QtWidgets.QLabel()
            label.setText(richtext("HOME",f"<html><head/><body><p style='margin-bottom:0; font-weight: bold;'>{rating} - {ratings_quote[rating]}</p><p style='margin: 0; padding-top: 0; font-size: 7pt; color: lightgrey;'>{value} SHOWS</p></body></html>"))
