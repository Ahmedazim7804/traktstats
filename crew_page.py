...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.directors, self.writers = parseData.most_watched_crew()
        ...
        DirectorsUi = Ui('DIRECTORS', self.directors)
        WritersUi = Ui('WRITERS', self.writers)

        self.scrollWidget.layout().addWidget(DirectorsUi)
        self.scrollWidget.layout().addWidget(WritersUi)
...
class Ui(QtWidgets.QWidget):
    def __init__(self, department, department_crew):
        ...
        self.department_crew = department_crew
...
""")
    ...
    def retranslateUi(self):
        ...
        self.label.setText(_translate("Form", f"MOST WATCHED {self.department}"))
    ...
    def next_page(self):
        self.page_number += 1
        department_crew = self.department_crew[self.page_number * 10: (self.page_number + 1) * 10]

        next_page_crew = self.department_crew[(self.page_number+1) * 10: (self.page_number + 2) * 10]
        if not next_page_crew:
            self.next_bt.setDisabled(True)
        if not self.previous_bt.isEnabled():
            self.previous_bt.setEnabled(True)

        page = CastPage(department_crew, startIndex=self.page_number * 10)
    ...
    def previous_page(self):
        ...
        previous_page_crew = self.department_crew[(self.page_number - 1) * 10: self.page_number * 10]
        if not previous_page_crew:
...
class CastPage(QtWidgets.QWidget):
    def __init__(self, department_crew, startIndex=0, parent=None):
        QtWidgets.QWidget.__init__(self, parent)
        ...
        for id, crew in department_crew:
            item = CastItem(crew, startIndex)
...
class CastItem(QtWidgets.QWidget):
    def __init__(self, crew, index, parent=None):
        QtWidgets.QWidget.__init__(self, parent)
        ...
        self.name = QtWidgets.QLabel(crew['name'].title())
        ...
        self.image_path = crew['image']

        self.movies = crew.get('movies', {})
        self.shows = crew.get('shows', {})
